...
* **Worker Throughput**: A single background worker processes **$\sim 25\text{ simulations/second}$** (for standard $1,000$-interaction workloads), delivering **$1,500\text{ simulations/minute}$** or **$90,000\text{ simulations/hour}$** ($\ge 16$ concurrent submissions tested successfully).
* **Queue Ingestion vs Worker Rate**: PostgreSQL handled burst ingestion at **$2,053\text{ jobs/second}$** ($0.48\text{ ms/job}$), demonstrating that queue growth is governed by worker compute throughput, not enqueue ingestion.
...
## 19. Phase 22: Production Cleanup Scheduling & Operational Lifecycle

Phase 22 makes simulation retention automatically recurring and production-safe using Render Cron:

* **Scheduling Architecture (Option A: Render Cron)**: A dedicated cron service defined in [`render.yaml`](render.yaml) (`type: cron`, name `behaviorsim-cleanup`) executes daily at **02:00 UTC** (`0 2 * * *`).
* **Command**: Ephemeral container executes `python -m app.cleanup` within the application environment.
* **Database & Environment Sharing**: Connects via `DATABASE_URL` linked securely from `behaviorsim-api` with zero credential duplication or hardcoded secrets.
* **Failure Semantics**: The cleanup CLI exits with non-zero status code (`1`) on unhandled failure so Render registers the failed run and issues alerts. Committed batches remain committed; the next scheduled run safely resumes.
* **Empty Run Safety**: Successful empty runs (`eligible=0, deleted=0`) exit cleanly with code `0`.
* **Zero Quota Refunds & Active Protection**: Cleanup strictly ignores `pending` and `running` jobs, never refunds quota, and deletes candidate rows by ID in bounded transactions (100 rows/batch) using `SKIP LOCKED`.
* **Storage Lifecycle**: PostgreSQL dead tuples generated by deletion are reclaimed and recycled by `autovacuum`, stabilizing table size into steady-state equilibrium without object storage.

## 20. Phase 23: Production Monitoring, Alerting & SLO Readiness

Phase 23 establishes production-grade operational observability, SLIs/SLOs, alerting thresholds, and incident runbooks:

* **SLI / SLO Framework**: Target $\ge 99.0\%$ monthly availability, submission latency p95 $< 150\text{ ms}$, status latency p95 $< 100\text{ ms}$, simulation completion rate $\ge 99.0\%$, and oldest pending queue age $< 60\text{ seconds}$.
* **Bounded Latency Histograms**: 10 empirical duration buckets (`<10ms` to `>5s`) tracked in-process without high-cardinality memory growth.
* **Queue Health & Age Telemetry**: `GET /v1/diagnostics` exposes `depth`, `oldest_pending_age_seconds`, and `oldest_running_age_seconds` backed by index queries under a 5-second TTL cache.
* **Stuck-Job & Worker Visibility**: Tracks stuck-job recoveries, retry exhaustions, completed interaction volume, and worker compute throughput.
* **Stale Cleanup Alerts**: Flags warning condition if retention cleanup has not succeeded within 26 hours.
